...
class WNOSpider(Spider):
    name = "WNO"
    SITE_NAME = 'welshnewspapersonline'
    ...
    for i in range(len(SEARCH_KEY_WORD_INFOS)):
        search = ArchiveSearch(archive="walsnews", search_text=SEARCH_KEY_WORD_INFOS[i]['keyword'],
                          archive_date_start=SEARCH_KEY_WORD_INFOS[i]['start_date(xxxx)'],
                          archive_date_end=SEARCH_KEY_WORD_INFOS[i]['end_date(xxxx)'],
                          search_batch_id="WNO")
        search_db.append(search)
        urls.append('http://newspapers.library.wales/search?range[min]=' + SEARCH_KEY_WORD_INFOS[i]['start_date(xxxx)'] + '&range[max]=' + SEARCH_KEY_WORD_INFOS[i]['end_date(xxxx)'] + '&query=' + SEARCH_KEY_WORD_INFOS[i]['keyword'] + '&page=1')
        ...
        if next_button != None:
            try:
                ...
                if next_url:
                    ...
                    yield scrapy.Request(next_url, meta=response.meta, callback=self.parse, headers=self.headers)
