...
class BNASpider(Spider):
    ...
    #INPUT_FILENAME = 'BNA_search_input.csv'
    ...
    for i in range(len(SEARCH_KEY_WORD_INFOS)):
        search_db.append(ArchiveSearch(archive="britishnewspaperarchive", search_text=SEARCH_KEY_WORD_INFOS[i]['keyword'],
                                       archive_date_start=SEARCH_KEY_WORD_INFOS[i]['start day(xxxx-xx-xx)'],
                                       archive_date_end=SEARCH_KEY_WORD_INFOS[i]['end day(xxxx-xx-xx)'],
                                       search_batch_id="BNA"))
