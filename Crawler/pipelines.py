    ...
    def process_item(self, PageItem, spider):
        ...
        if spider.name == 'BNA':
            ...
            for i in range(articles_in_page_count):
                ...
                search_id   = PageItem['search_id'][i]
                ...
                article_item.writeIntoDataBase()
            if articles_in_page_count > 0:
                dbutils.write_new_search_results(PageItem['search_id'][0], filename)
            ...
            for i in range(articles_in_page_count):
                ...
                article_item = ArticleItem(site=site, keyword=keyword, title=title, publish=publish, description=description, type_=type_,
                    word=words, newspaper=newspaper, page=page, download_page=download_page, search_id=search_id,
                                           download_url=download_page, ocr=ocr, start_date=start_date, end_date=end_date)
                
                print 'Writting the data into the json file now..........'
                filename = "{}_{}_{}_{}".format(site, keyword, start_date, end_date)
                article_item.writeIntoJsonFile(filename)
                article_item.writeIntoDataBase(site='WNO')
            if articles_in_page_count > 0:
                dbutils.write_new_search_results(PageItem['search_id'][0], filename)
...
class ArticleItem:
    ...
    def writeIntoDataBase(self, site='BNA'):
        ...
        if site == 'BNA':
            ...
            publication_date = '{0.year:4d}-{0.month:02d}-{0.day:02d}'.format(publication_date)
            self.county = ""
        searchresult = ArchiveSearchResult(title = self.title.encode('latin-1', 'ignore'),
